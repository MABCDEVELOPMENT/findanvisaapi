...
public class FindDataCosmeticRegister {
	...
	@Autowired
	public void setService(CosmeticRegisterRepository cosmeticRegisterRepository, ProcessRepository processRepository) {
	...
	public static List<ContentCosmeticRegister> filter(QueryRecordParameter queryRecordParameter) {
		...
		return cosmeticRegisterRepository.findAll(new Specification<ContentCosmeticRegister>() {
			...
			@Override
			public Predicate toPredicate(Root<ContentCosmeticRegister> root, CriteriaQuery<?> query,
				...
				if (queryRecordParameter.getAuthorizationNumber() != null
					...
					predicates.add(criteriaBuilder
							.and(criteriaBuilder.equal(root.get("contentCosmeticRegisterDetail").get("autorizacao"),
